    ...
    def draw(self, screen):
        if self.is_hovered_flag:
            current_radius = self.radius + \
                min((time.time() - self.hover_time) * 10, self.radius * 0.5)
        else:
            current_radius = self.radius
        ...
        if self.animation_start_time is not None:
            ...
            if elapsed_time < 0.5:
                ...
                if self.visible:
                    self.center = (
                        int((1 - t) * SCREEN_SIZE[0] //
                            2 + t * self.final_pos[0]),
                        int((1 - t) * SCREEN_SIZE[1] //
                            2 + t * self.final_pos[1])
                    )
                else:
                    self.center = (
                        int(t * SCREEN_SIZE[0] // 2 +
                            (1 - t) * self.final_pos[0]),
                        int(t * SCREEN_SIZE[1] // 2 +
                            (1 - t) * self.final_pos[1])
                    )
            else:
                self.center = self.final_pos if self.visible else (
        ...
        if self.visible or self.is_animating:
            if self.image:
                top_left = (self.center[0] - self.radius,
                            self.center[1] - self.radius)
                screen.blit(self.image, top_left)
            else:
                pygame.draw.circle(screen, NAVY_BLUE,
                                   self.center, int(current_radius))
            pygame.draw.circle(screen, LIGHT_BLUE,
                               self.center, int(current_radius), 5)
...
def create_circles():
    ...
    main_circle = AppCircle(
        (center_x, center_y), main_circle_radius, 0, (center_x, center_y), is_main=True)
    ...
    for i in range(num_circles):
        ...
        circles.append(AppCircle((center_x, center_y),
                       app_circle_radius, i + 1, (x, y)))
    return circles
                    ...
                    if circle.is_hovered((screen_x, screen_y)):
                        ...
                        if circle.is_main:
                            ...
                            if time.time() - last_toggle_time > HOME_TOGGLE_DELAY:
                                ...
                                print(f"Toggling apps visibility to: {
                                      apps_visible}")
                            ...
                            if time.time() > last_app_select_time:
                                print(f"Circle {circle.app_index} hovered with visibility {
                                      circle.visible}")
                                try:
