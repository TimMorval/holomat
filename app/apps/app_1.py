...
def run(screen, camera_manager):
    ...
    clear_button_rect = pygame.Rect(
        (SCREEN_SIZE[0] // 2 - 150, SCREEN_SIZE[1] - 150, 300, 70))
    ...
    while running:
        ...
        if transformed_landmarks:
            for hand_landmarks in transformed_landmarks:
                thumb_pos = (int(hand_landmarks[4][0]), int(
                    hand_landmarks[4][1]))  # THUMB_TIP
                index_pos = (int(hand_landmarks[8][0]), int(
                    hand_landmarks[8][1]))  # INDEX_FINGER_TIP

                mid_point = ((thumb_pos[0] + index_pos[0]) //
                             2, (thumb_pos[1] + index_pos[1]) // 2)
        ...
        pygame.draw.circle(screen, NAVY_BLUE,
                           home_button_center, home_button_radius)
        pygame.draw.circle(screen, LIGHT_BLUE,
                           home_button_center, home_button_radius, 5)
        font = pygame.font.Font(None, 36)
        ...
        # Draw clear button
        pygame.draw.rect(screen, NAVY_BLUE,
                         clear_button_rect, border_radius=15)
        pygame.draw.rect(screen, LIGHT_BLUE,
                         clear_button_rect, 5, border_radius=15)
